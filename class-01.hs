...
{-
   ...
   расстановку скобок:
    ...
    5 < 7 || 10 > 3  True
    sqrt (-2)    NaN
    sqrt (sqrt 16)  = 1.8612097182041991
    let x = 4 in (sin x)^2 + (cos x)^2  = 1.0
    x   Not in scope: `x'  x не был определен
...
-- (объясните смысл типовой аннотации: результат выражения будет принадлежать типу а, так как аргумент принадлежит типу ф)
double :: Num a => a -> a
double a = a * 2
...
max3 :: Ord a => a -> a -> a -> a
max3 a b c = if (max a b) > (max b c) then (max a b) else (max b c)
